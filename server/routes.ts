...
export async function registerRoutes(
  ...
  // === Force Seed Indian Products ===
  // This ensures Indian products are always available
  seedIndianProducts().catch(err => {
    console.error("Failed to seed Indian products (non-fatal):", err.message);
  });
...
async function seedIndianProducts() {
  const products_list = await storage.getProducts();  
  // Always seed Indian products - update existing or add new ones
  ...
  for (const p of indianProducts) {
    const existingProduct = products_list.find(prod => prod.sku === p.sku);
    if (existingProduct) {
      // Update existing product
      await storage.updateProduct(existingProduct.id, p);
    } else {
      // Create new product
      await storage.createProduct(p);
    }
  ...
  // Seed Customers
  const customers_list = await storage.getCustomers();
  const indianCustomers = [
    { name: "Walk-in Customer", phone: "0000000000", email: "" },
    { name: "Rahul Sharma", phone: "9123456780", email: "rahul@example.in" },
    { name: "Priya Sharma", phone: "9876543210", email: "priya@example.in" },
    { name: "Suresh Patel", phone: "9876543211", email: "suresh@example.in" },
    { name: "Anita Reddy", phone: "9876543212", email: "anita@example.in" },
    { name: "Vikram Singh", phone: "9876543213", email: "vikram@example.in" },
  ];

  for (const c of indianCustomers) {
    const existingCustomer = customers_list.find(cust => cust.phone === c.phone);
    if (!existingCustomer) {
